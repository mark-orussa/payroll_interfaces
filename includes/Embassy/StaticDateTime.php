...
use DateTime, DateTimeZone, PDOException;

class StaticDateTime {
	/**
	 * DateTime methods to make it easier to do simple things.
	 *
	...
	public static function addTime($initDate, $length, $period) {
		/**
		 * This will add time to a date. If adding a month it will do so without rolling over to the next month if the new date doesn't exist. Example: going from March 31 to April 30.
		 *
		...
		$startDay = $dateTime->format('j'); // The day.
		$Debug->add('$startDay:' . $startDay);
		if( $period == 'hour' ){
		...
		if( $period == 'month' && $startDay != $end_day ){ // If the day is different, set it to the last day of the month.
...
<select name="timeZoneSelect" id="timeZoneSelect" data-mini="false" data-inline="true">';
	...
	public static function convertToDateTime($str) {
		/**
		 * Convert something into a DateTime object. This relies on strtotime, which accepts string formats described here:
		 * http://php.net/manual/en/datetime.formats.php
		 *
		 * @param    object string    $str    A date string, timestamp, or DateTime object recognized by strtotime().
		 *
...
FROM
	...
	public function mysqlDateTime($datetime) {
		/**
	...
	public static function subtractTime($initDate, $length, $period) {
		/**
		 * This will add time to a date.
		 *
		 * @param    object|string $initDate A DateTime object or string. Strings will be converted to a DateTime object.
		 * @param    integer $length         The number of years or months to add.
		 * @param    string $period          A time period of 'year' or 'month'.
		 *
		 * @return    object    Returns a DateTime object.
...
//		$startDay= $dateTime->format('j');//The day.
	...
	public static function utcToLocal($datetime = false, $useSessionFormat = true, $timeZone = false) {
		 ...
		 * $newTime = Time::utcToLocal(false,false);
		 *
		 * @param    string|object $datetime  Any English textual date or DateTime object.
		...
		global $Debug, $Message;
		if( empty($timeZone) ){
			// If there is no default time format, make one. Otherwise use the one defined in $_SESSION.
			if( empty($_SESSION['timeZone']) ){
		...
		if( $useSessionFormat ){
			// If there is no default time format, make one. Otherwise use the one defined in $_SESSION.
			return empty($_SESSION['dateFormat']) ? $datetime->format('l F jS, Y h:i:s A') : $datetime->format($_SESSION['dateFormat']);
		}else{
			return $datetime;
		}
	}
